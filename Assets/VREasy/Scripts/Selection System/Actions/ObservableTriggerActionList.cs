    ...
    public class ObservableTriggerActionList : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (targetComponent != null && !string.IsNullOrEmpty(targetProperty))
            {
                ...
                observedType = targetComponent.GetType().GetTypeInfo().GetDeclaredProperty(targetProperty).PropertyType;
#else
        ...
        {
#if NETFX_CORE || UNITY_WSA_10_0
            T value = (T)targetComponent.GetType().GetTypeInfo().GetDeclaredProperty(targetProperty).GetValue(targetComponent, null);
#else
