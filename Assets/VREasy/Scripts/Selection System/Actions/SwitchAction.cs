    ...
    public class SwitchAction : VRAction
    {
        ...
        private void switchValues(string propertyName, bool isProperty = true)
        {
            ...
            try
            {
                ...
                if (isProperty)
                    Target.GetType().GetTypeInfo().GetDeclaredProperty(propertyName).SetValue(Target, swapObjects[_swapIndex], null);
                else
                    Target.GetType().GetTypeInfo().GetDeclaredField(propertyName).SetValue(swapObjects[_swapIndex], null);
#else
                if (isProperty)
