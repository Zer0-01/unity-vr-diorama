    ...
    public class VRGrabbable : VRElement
    {
                ...
                case GRAB_TYPE.ROTATE:
                    {
                        ...
                        selectorOffset_rotation = selectorTransform.rotation;
                        selectorTransformOffset_position = selectorTransform.position;
                        if (isGrabbed) return;
        ...
        void FixedUpdate()
        {
            if (isGrabbed)
            {
                ...
                if (type == GRAB_TYPE.SLIDE && selectorTransform != null || dragKinematic)
                {
                    ...
                    float moveX = xAxis ? Vector3.Dot(moved, transform.right) : 0;
                    float moveY = yAxis ? Vector3.Dot(moved, transform.up) : 0;
                    float moveZ = zAxis ? Vector3.Dot(moved, transform.forward) : 0;
                ...
                else if (type == GRAB_TYPE.ROTATE && selectorTransform != null)
                {
                    ...
                    if (!usePosition)
                    {
                        ...
                        Quaternion diff = Quaternion.Inverse(selectorOffset_rotation) * selectorTransform.rotation;
                    ...
                    } else
                    {
                        ...
                        //diff = Quaternion.Inverse(Quaternion.LookRotation(selectorTransformOffset_position - transform.position)) * Quaternion.LookRotation(selectorTransform.position - transform.position);
                        //diffMultiplier *= -translationScale;
                    ...
                    if (constraintMovement)
                    {
                        Quaternion futureRotation = transform.localRotation * finalRotation;


                        float angleDifference = -GetSignedAngle(futureRotation, originalRotation, transform.TransformDirection(rotateAround));
                        if (angleDifference <= maxRotation && angleDifference >= minRotation)
                        {
                            // within range, allow rotation
                            transform.rotation = transform.rotation * finalRotation;
                        }
        ...
        private void snapObjectToOrigin()
        {
            if(snapToClosest)
            {
                ...
                float currentDistance = snapToOrigin ? Vector3.Distance(transform.position, originalPosition) : int.MaxValue;
                int currentSnapPosition = snapToOrigin ? -1 : -2;
                for (int ii=0; ii < snapPositions.Count; ii++)
                {
                    float dist = Vector3.Distance(snapPositions[ii].position, transform.position);
                ...
                {
                    transform.position = snapPositions[currentSnapPosition].position;
                    transform.rotation = snapPositions[currentSnapPosition].rotation;
                }
            } else
            {
                // snap only if near a snapping position
                if (snapToOrigin && Vector3.Distance(transform.position, originalPosition) <= snapRadius)
                {
                    transform.position = originalPosition;
                    transform.rotation = originalRotation;
                }
                ...
                foreach (Transform t in snapPositions)
                {
                    if (Vector3.Distance(transform.position, t.position) <= snapRadius)
                    {
